...
-- ============================================
-- STUDY MATERIAL EMBEDDINGS TABLE (pgvector RAG)
-- ============================================
...
CREATE INDEX idx_embeddings_file_path ON study_material_embeddings(file_path);
CREATE INDEX idx_embeddings_course_code ON study_material_embeddings(course_code);
CREATE INDEX idx_embeddings_vector ON study_material_embeddings
...
CREATE POLICY "Allow authenticated users to read embeddings"
    ON study_material_embeddings FOR SELECT TO authenticated USING (true);

CREATE POLICY "Allow service role full access to embeddings"
    ON study_material_embeddings FOR ALL TO service_role USING (true) WITH CHECK (true);
...
RETURNS TABLE (
    ...
    course_code TEXT, level TEXT, similarity FLOAT
)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
BEGIN
    RETURN QUERY
    SELECT sme.id, sme.file_path, sme.content, sme.course_code, sme.level,
           1 - (sme.embedding <=> query_embedding) AS similarity
    FROM study_material_embeddings sme
    WHERE (1 - (sme.embedding <=> query_embedding)) > match_threshold
